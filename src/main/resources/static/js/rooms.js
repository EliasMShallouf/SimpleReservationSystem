...
$(function() {
    ...
    function editRoom(room) {
        ...
        $('#eroom_des').val(room.description);
        $('#eroom_cap').val(room.maxPlacesCount);
    ...
    function room() {
        ...
        let description = $('#room_des').val();
        let maxPlacesCount = $('#room_cap').val();

        doRoom(
            JSON.stringify({ name, description, maxPlacesCount }),
            () => {
                    ...
                    $('#room_table tbody')
                      .append(
                        $('<tr id=\'room_' + r.id + '\' scope=row>')
                            ...
                            .append($('<td>').text(r.maxPlacesCount))
                            .append(
                                $('<td>')
                                    .append(
                                        $('<button>')
    ...
    $('#edit_room_form').submit(function(event) {
        ...
        let description = $('#eroom_des').val();
        let maxPlacesCount = $('#eroom_cap').val();

        updateRoom(
            JSON.stringify({ id, name, description, maxPlacesCount }),
            () => {
                alert('Room has edited successfully');

                /*let targetRow = '#room_' + id;
                $(targetRow).find('td').eq(0).text(name);
                $(targetRow).find('td').eq(1).text(description);
                $(targetRow).find('td').eq(2).text(maxPlacesCount);*/
                reload();
